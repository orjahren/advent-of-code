...
while len(q) > 0:
    ...
    if x:
        # if type(x) == dict:
        # if list(x.values())[0] != False:new
        next_q = [v for v in list(x.keys()) if str(v) not in ["META", "files"]]
        print("Legger dette i q:", next_q)
        q.extend(next_q)
        # else:
        ...
        print("x er none, så dette må være et løv-dir")
        print(prev)
        prev = prev
