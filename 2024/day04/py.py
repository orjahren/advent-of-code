...
def part1(grid):
    return sum(1 for r, c in get_letter_pos(grid, "X")for dr in (-1, 0, 1)for dc in (-1, 0, 1)if (dr != 0 or dc != 0) and cand_is_in_bounds(r + 3 * dr, c + 3 * dc, grid) and grid[r + dr][c + dc] == "M" and grid[r + 2 * dr][c + 2 * dc] == "A" and grid[r + 3 * dr][c + 3 * dc] == "S")
