...
#include <vector>
using namespace std;
void addAndExhaust(vector<vector<string>> &target, string &line)
{
  target.push_back(vector<string>());
  do
  {
    target.back().push_back(line);
...
pair<vector<vector<string>>, vector<vector<string>>> readInput()
{
  ...
  while (getline(cin, line))
    addAndExhaust(line[0] == '#' ? locks : keys, line);
  return {locks, keys};
}
